...
- Validated locally on 2026-03-31 for outside-in selection replay:
  - `discover-ghidra.sh --print-install-dir` resolved
  ...
  - the local shell toolchain exposed `java 17.0.18` and `javac 17.0.18`
    while Ghidra headless replay itself used the bundled Java 21 runtime
  - `bash -n .agents/skills/ghidra-headless/scripts/run-headless-analysis.sh`
  ...
  - `review-evidence` and `target-selection` executed the updated Java scripts
    successfully through Ghidra headless and emitted runtime artifacts under
  ...
  - Ghidra emitted non-fatal cache-maintenance and packed-db cache permission
    warnings under `/var/tmp/yuyang-ghidra/`, but the supported actions still
...
```bash
export SKILL_ROOT=/path/to/installed/ghidra-headless
java -version
javac -version
bash .agents/skills/ghidra-headless/scripts/run-headless-analysis.sh --help >/dev/null 2>&1 || true
bash -n .agents/skills/ghidra-headless/scripts/run-headless-analysis.sh
...
- `bash -n .agents/skills/ghidra-headless/scripts/run-headless-analysis.sh`
  passed
- install discovery resolved `/opt/homebrew/Cellar/ghidra/12.0.4/libexec`
- install discovery resolved
  `/opt/homebrew/Cellar/ghidra/12.0.4/libexec/support/analyzeHeadless`
- `discover-ghidra.sh --show-help` succeeded for the resolved install
- candidate sample binary exists at
  `.work/ghidra-artifacts/sample-target-archive-runtime/normalized-members/sample-target--archive_main.o`
- Ghidra headless replay used the install-bundled Java 21 runtime successfully
...
- non-fatal cache-maintenance and packed-db cache permission warnings were
  emitted under `/var/tmp/yuyang-ghidra/`, but the supported actions still
  passed
...
such as `.agents/skills/ghidra-headless`,
`.claude/skills/ghidra-headless`, or an absolute global install
path.
...
| Install path                                                  | `/opt/homebrew/Cellar/ghidra/12.0.4/libexec`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Resolved analyzeHeadless path                                 | `/opt/homebrew/Cellar/ghidra/12.0.4/libexec/support/analyzeHeadless`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Source-comparison real-reference happy-path behavior observed | `pass; Ghidra baseline against /Users/yuyang/Projects/wimal-build/build/llvm/bin/minigzip now aligns with tracked zlib review files under third_party/upstream/zlib/`                                                                                                                                                                                                                                                                                                                                                                                                 |
