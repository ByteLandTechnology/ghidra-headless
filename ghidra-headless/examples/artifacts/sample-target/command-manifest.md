...
- project-local install: `.agents/skills/ghidra-headless`
- Claude-style project-local install: `.claude/skills/ghidra-headless`
- global install: `/absolute/path/to/ghidra-headless`
...
- the current sample validation path targets
  `/opt/homebrew/Cellar/ghidra/12.0.4/libexec/support/analyzeHeadless`
...
The repository does not yet ship a generic
`run-headless-analysis.sh --action compare-selected` wrapper. Record the exact
target-specific build and run commands in `comparison-command-log.md`.
